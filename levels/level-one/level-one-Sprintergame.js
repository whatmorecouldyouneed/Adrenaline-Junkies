...
function setup() {
  ...
  var aiRunner = new Sprinter(random(0.03) + 0.06, opponentColor); // set AI runner
  for (var i = 1; i < SPRINTER_COUNT; i++) {
      // push opponents
      if(i != Math.floor(SPRINTER_COUNT/2)) {
          runners.push(new Sprinter(random(0.05) + 0.07, opponentColor));
      } else {
          runners.splice(Math.floor(runners.length / 2), 0, aiRunner);
      }
  }
